...
useEffect(() => {
  if (userData) {
    setEditForm({
      ...
      phoneNumber: userData.phoneNumber || "",
       photoURL: userData.profileImage || "",  // keep this for edit form preview
    });
  }
}, [userData]);
  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      // Assign axios response to a variable
      ...
      setUserData(response.data.user); // ✅ now response is defined
          ...
          <TabsHeader className="bg-white rounded-xl p-1 shadow-md">
            {tabs.map(({ label, value, icon: Icon, color }) => (
              <Tab
                key={value}
                value={value}
                className="rounded-lg flex items-center gap-2 font-medium"
              >
